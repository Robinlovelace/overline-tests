...
The aim of this repo is to test functionality in the `overline` function and speed-it up.
...
`sfheaders` is 2x faster:

```{r}
sl_geos = geos::as_geos_geometry(sl)
bench::mark(check = FALSE,
  sf = {c1 = sf::st_coordinates(sl)},
  sfheaders = {c1_new = sfheaders::sf_to_df(sl)}
  # geos = {c1_geos = data.frame(geos::geos_x(sl_geos), geos::geos_y(sl_geos))}
)
...
`kit` is 3x faster at finding duplicates:

```{r}
l1_start = duplicated(l1) # find the break points between lines
l1_start_kit = kit::fduplicated(l1)
identical(l1_start, l1_start_kit)
l1_start = c(l1_start[2:length(l1)], FALSE)
bench::mark(
  old = {l1_start = c(duplicated(l1)[2:length(l1)], FALSE)},
  new = {l1_start_new = c(kit::fduplicated(l1)[2:length(l1)], FALSE)}
)
