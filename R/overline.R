...
overline3 <-
  function(sl, attrib, fun = "sum", simplify = TRUE, quiet = FALSE) {
    ...
    sln <- sf::st_drop_geometry(sln)
    l1 <- c1[["linestring_id"]] # Get which line each point is part of
    c1 <- cbind(c1[["x"]], c1[["y"]])
    l1_start <- kit::fduplicated(l1) # find the break points between lines
    l1_start <- c(l1_start[2:length(l1)], FALSE)
    c2 <- c1[2:nrow(c1), 1:2] # Create new coords offset by one row
    c2 <- rbind(c2, c(NA, NA))
    c2[nrow(c1), ] <- c(NA, NA)
    ...
    c3 <- cbind(c1, c2) # make new matrix of start and end coords
    c3 <- c3[!is.na(c3[, 3]), ]
    slnr <- sln[l1[l1_start], , drop = FALSE] # repeate attributes
    c3n <- flip_direction(c3)
    # waldo::compare(c3, c3n)

    # if (!quiet) {
    #   message(paste0(Sys.time(), " removing duplicates"))
    # }
    slnrc <- cbind(c3n, slnr) |>
      as.data.frame()
    sls <- dplyr::group_by(slnrc, `1`, `2`, `3`, `4`)
    sls <- dplyr::ungroup(dplyr::summarise_all(sls, .funs = fun))
    coords <- as.matrix(sls[, 1:4])
    res_attributes <- sls[, -c(1:4)]

    # # Make Geometry
    # res_geometry <- sf::st_as_sfc(
    #   if (requireNamespace("pbapply", quietly = TRUE)) {
    #     pbapply::pblapply(1:nrow(coords), function(y) {
    #       sf::st_linestring(matrix(coords[y, ], ncol = 2, byrow = T))
    #     })
    ...
    if (simplify) {
        ...
        overlined_simple <- ol_grp(res_exploded, attrib)
      }

      overlined_simple = sf::st_as_sf(
        sf::st_drop_geometry(overlined_simple),
        geometry = sf::st_geometry(overlined_simple)
      )
...
#' @examples
#' x_o = matrix(c(0, 0, 1, 1, 0, 1, 1, 0), ncol = 2, byrow = TRUE)
#' x_d = matrix(c(0, 1, 1, 0, 0, 0, 1, 1), ncol = 2, byrow = TRUE)
#' x = cbind(x_o, x_d)
#' x_flipped_old = flip_direction_old(x)
#' x_flipped = flip_direction(x)
#' waldo::compare(x_flipped_old, x_flipped)
#' bench::mark(f_old = flip_direction_old(x), f_new = flip_direction(x))
flip_direction_old = function(x) {
  t(apply(x, MARGIN = 1, FUN = function(y) {
      if (y[1] != y[3]) {
        if (y[1] > y[3]) {
          c(y[3], y[4], y[1], y[2])
        } else {
...
flip_direction = function(x) {
  ...
  x_o_sum = rowSums(x_o)
  x_d_sum = rowSums(x_d)
  x_o_larger = x_o_sum > x_d_sum
  x[x_o_larger, 1:2] = x_d[x_o_larger, ]
  x[x_o_larger, 3:4] = x_o[x_o_larger, ]
  x
}
